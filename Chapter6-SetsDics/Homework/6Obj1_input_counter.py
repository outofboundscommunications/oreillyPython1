...
while True:
  #initialize length of dict and wordset
  myDicLength = len(myDic)
  myWordSetLength = len(myWordSet)
  ...
  for word in words:
      ...
      myWordSet.add(word)
      #If the set increases in size
      if len(myWordSet)>myWordSetLength:
          #add the word to the dict a key with the value being the new length of the set
          myDicLength +=1
